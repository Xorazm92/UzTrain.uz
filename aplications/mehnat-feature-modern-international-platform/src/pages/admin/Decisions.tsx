...
export default function Decisions() {
  ...
  const [deletingItem, setDeletingItem] = useState<Decision | null>(null);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);

  const { register, handleSubmit, reset, setValue, watch } = useForm<DecisionFormData>();
  ...
  const handleEdit = (item: Decision) => {
    setEditingItem(item);
  ...
  const handleDelete = (item: Decision) => {
    setDeletingItem(item);
    setDeleteOpen(true);
  };

  const onSubmit = async (formData: DecisionFormData) => {
    try {
      ...
      if (editingItem) {
        ...
        if (error) throw error;
        toast.success('Qaror muvaffaqiyatli yangilandi');
      } else {
        const { error } = await supabase
          .from('qarorlar')
          .insert([decisionData]);
        ...
        if (error) throw error;
        toast.success('Qaror muvaffaqiyatli qo\'shildi');
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success('Qaror muvaffaqiyatli o\'chirildi');
      setDeleteOpen(false);
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <DataTable
        ...
        searchKey="titleblob"
        onAdd={handleAdd}
        addButtonText="Yangi Qaror Qo'shish"
        title="Qarorlar"
      />
      ...
      <FormDialog
        ...
        title={editingItem ? 'Qaror Tahrirlash' : 'Yangi Qaror Qo\'shish'}
        description="Qaror ma'lumotlarini kiriting"
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Qaror sarlavhasini kiriting"
            />
          </div>
          ...
          <div>
            ...
            <Textarea
              id="content"
              {...register('content')}
              placeholder="Qaror mazmunini kiriting"
              rows={4}
            />
          </div>
      ...
      <ConfirmDialog
        ...
        title="Qarorni o'chirish"
        description={`"${deletingItem?.titleblob}" qarorini o'chirishni tasdiqlaysizmi? Bu amalni bekor qilib bo'lmaydi.`}
